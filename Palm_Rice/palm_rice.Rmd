...
grid_dim= 
  merge(grid_dim,ngrid)%>%
  mutate(surf_grid= area_grid/ngrid,
...
plane_df_step= 
  ...
  dplyr::mutate(Date= date,
                irradiation= Global_Intercepted/area,
                irradiance= irradiation/timestep)
# Global intercepted radiation (irradiation) in MJ m-2 timestep-1
# Irradiance: MJ m-2 s-1 

grid_df_day= 
  plane_df_step%>%
  group_by(Treatment,id)%>%
  summarise(Date= mean(.data$date),
            Intercepted= sum(.data$Global_Intercepted/.data$area),
            # Global intercepted radiation in MJ m-2 d-1
